...
async function ensureSeriesContext(seriesUrl, fallbackTitle = null) {
        ...
        if (!fs.existsSync(seriesMetaPath)) {
                ...
                console.log(`   💾 Saved metadata -> ${path.relative(process.cwd(), seriesMetaPath)}`);
        }
...
function saveEpisodeToDisk(seriesCtx, seasonNumber, episodeNumber, payload) {
        ...
        updateLatestEpisodesFile(seriesCtx, seasonNumber, episodeNumber);
}
...
function getExistingEpisodesInSeason(seriesCtx, seasonNumber) {
        const seasonDir = path.join(seriesCtx.seriesDir, `season-${seasonNumber}`);
        ...
        const episodes = files
                ...
                .map(f => parseInt(f.replace('episode-', '').replace('.json', '')))
                .filter(n => !isNaN(n))
                .sort((a, b) => a - b);
...
async function ensureSeasonBackfilled(seriesCtx, seasonNumber, force = false) {
        ...
        const existing = getExistingEpisodesInSeason(seriesCtx, seasonNumber);
        ...
        const afterFetch = getExistingEpisodesInSeason(seriesCtx, seasonNumber);
                ...
                if (existingCount === 0) {
                        ...
                        if (fs.existsSync(seasonDir)) {
                                try {
                                        const files = fs.readdirSync(seasonDir);
                                        if (files.length === 0) {
                                                console.log(`   🗑️  Removing empty season folder: season-${seasonNumber}`);
                                                fs.rmdirSync(seasonDir);
                                        }
                                } catch (err) {
                                        console.log(`   ⚠️ Could not remove empty folder: ${err.message}`);
                                }
                        }
                }
...
function updateLatestEpisodesFile(seriesCtx, seasonNumber, episodeNumber) {
        ...
        const seriesSlug = seriesCtx.baseName;
        
        let episodeThumbnail = seriesCtx.common.thumbnail || null;
        try {
                const { filePath } = episodeFilePath(seriesCtx, seasonNumber, episodeNumber);
                if (fs.existsSync(filePath)) {
        ...
        const episodeEntry = {
                title: `${seriesCtx.title} - Episode ${episodeNumber}`,
        ...
        latest = latest.filter(e => !(e.seriesSlug === seriesSlug && e.season === seasonNumber && e.episode === episodeNumber));
        latest.unshift(episodeEntry);
        latest.sort((a, b) => new Date(b.fetchedAt) - new Date(a.fetchedAt));
